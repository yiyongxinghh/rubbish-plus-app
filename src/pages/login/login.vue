  ...
  <scroll-view :scroll-y="true" class="login-container">
    <view class="panel">
      <view class="login-title">
        <image src="@/static/images/rubbish-white.png" mode="aspectFit" class="login-icon" />
        <view>
          <text id="typing"></text>
        </view>
      </view>
      <view class="scroll" v-if="!isRegister">
        <view class="login-content" v-if="!isCodeLogin">
          <view class="res">
            <view>还没有帐号？<text @click="isRegister = !isRegister">注册</text></view>
          </view>
          <input class="login-input email" v-model="formData.userEmail" />
          <input class="login-input pass" password v-model="formData.userPass" />
        </view>
        <view class="login-content" v-else>
          <input class="login-input email" v-model="formData.userEmail" />
          <view class="login-code">
            <input class="login-input code" v-model="code" />
            <button @click="sendCode" :disabled="isSendCode" :class="{ send: isSendCode }">{{ isSendCode ?
    `code(${codeTime})`
    : 'code' }}</button>
          </view>
        </view>
        <view class="login-button">
          <button class="login-btn handel" @click="isCodeLogin = !isCodeLogin">切换</button>
      ...
      <view class="scroll" v-else>
        <view class="res-content" v-if="!isCodeLogin">
          <view class="res">
            <view>已经有帐号了. <text @click="isRegister = !isRegister">登录</text></view>
          </view>
...
const login = async (isCodeLogin: boolean) => {
  if (isCodeLogin) {
    try {
      const { data } = await codeVerificationLoginAPI(formData.userEmail, code.value)
    ...
    try {
      const { data } = await emailLoginAPI(formData.userEmail, formData.userPass)
      userStore.saveUserData(data.user)
      userStore.saveToken(data.access_token)
      showToast('登录成功', true)
      uni.switchTab({
